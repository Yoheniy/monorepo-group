...
export default function Injuries() {
  ...
  const fetchInjuries = async () => {
    try {
      ...
      const injRes = await fetch(`${API_BASE_URL}/injuries`, {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Participants
        const partRes = await fetch(`${API_BASE_URL}/participants`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleUpdateInjury = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/injuries/${editingInjury.id || editingInjury._id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_BASE_URL}/injuries/${deleteConfirmation.id || deleteConfirmation._id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
