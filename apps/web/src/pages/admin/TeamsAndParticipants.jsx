...
export default function TeamsAndParticipants() {
  ...
  useEffect(() => {
    ...
    const fetchTournamentData = async () => {
      ...
      try {
        ...
        const teamRes = await fetch(`${API_BASE_URL}/teams?tournamentId=${selectedTournamentId}`, {
          headers: { 'Authorization': `Bearer ${token}` },
        });
        ...
        const partRes = await fetch(`${API_BASE_URL}/participants?tournamentId=${selectedTournamentId}`, {
          headers: { 'Authorization': `Bearer ${token}` },
        });
        ...
        const coachesRes = await fetch(`${API_BASE_URL}/users?role=coach`, {
          headers: { 'Authorization': `Bearer ${token}` },
        });
        if (coachesRes.ok) {
  ...
  const handleAddParticipant = async (e) => {
    ...
    try {
      ...
      if (!userId) {
        ...
        const tmRes = await fetch(`${API_BASE_URL}/team-members`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
  ...
  const handleEditParticipant = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_BASE_URL}/participants/${selectedParticipant.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
